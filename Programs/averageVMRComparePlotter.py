...
def averageVMRComparePlotter(pair,instruments,folder,year):
    ...
    for i in range(0,len(cm),1): # Each element contains two elements
       ...
       curr_nd1_err=[]
       curr_nd2_err=[]
       if type(((cm[i])[0]).altitude)==list and type(((cm[i])[1]).altitude)==list and type(((cm[i])[0]).o3_vmr)==list and type(((cm[i])[1]).o3_vmr)==list and (type((cm[i])[0].is_retrieved)==list) and (type((cm[i])[1].is_retrieved)==list) and (type((cm[i])[0].is_retrieved)==list) and (type((cm[i])[1].is_retrieved)==list) and len((((cm[i])[0]).altitude))==201 and len((((cm[i])[1]).altitude))==201 and len((((cm[i])[0]).o3_vmr))==201 and len((((cm[i])[1]).o3_vmr))==201 and len((((cm[i])[0]).is_retrieved))==201 and len((((cm[i])[1]).is_retrieved))==201 and len((((cm[i])[0]).o3_vmr_error))==201 and len((((cm[i])[1]).o3_vmr_error))==201 :
          
          curr_alt1 = list(((cm[i])[0]).altitude) # Current altitude range #1 
          curr_nd1 = ((cm[i])[0]).o3_vmr # Current VMR profile #1
          curr_alt2 = list(((cm[i])[1]).altitude) # Current altitude range #2
          curr_nd2 = ((cm[i])[1]).o3_vmr
          curr_nd1_err = cm[i][0].o3_vmr_error # Current VMR error profile #1
          curr_nd2_err = cm[i][1].o3_vmr_error 
          curr_retrievals1 = cm[i][0].is_retrieved # Current retrieval documentation
       ...
       while k <= maxalt:
          ...
          if abs(curr_nd1[idx1])<1 and abs(curr_nd2[idx2])<1 and curr_retrievals1[curr_idx]==1 and curr_retrievals2[curr_idx]==1:
    ...
    axs[0].legend(['ACE-MAESTRO-UV','ACE-MAESTRO-VIS'],loc='upper left')
    ...
    filenamepng = title + ' ACE-MAESTRO UV & VIS, errorbars '+str(year)+'.png' # Create name for png plot file.
