...
def vmrAnalysis_v313(cm_pair,cm):
   if type(cm)==list:
       ...
       for i in range(0,len(cm),1):
          # print('1',((((cm[i])[0]).common_altitude)))
          # print('2',((((cm[i])[1]).altitude)))
          # print('3',((((cm[i])[0]).common_o3_vmr)))
          # print('4',((((cm[i])[1]).o3_vmr)))
          # print('5',((((cm[i])[1]).is_retrieved)))
          # print('6',((((cm[i])[0]).common_o3_vmr_error)))
          # print('7',((((cm[i])[1]).o3_vmr_error)) )
          if (type((cm[i])[0].datetime)==datetime.datetime) and (type((cm[i])[1].datetime)==datetime.datetime) and type(((cm[i])[0]).altitude)==list and type(((cm[i])[1]).altitude)==list and type(((cm[i])[0]).common_o3_vmr)==list and type(((cm[i])[0]).common_o3_vmr_error)==list and type(((cm[i])[1]).o3_vmr)==list and (type((cm[i])[0].is_retrieved)==list) and (type((cm[i])[1].is_retrieved)==list) and (type((cm[i])[0].is_retrieved)==list) and (type((cm[i])[1].is_retrieved)==list) and len((((cm[i])[0]).common_altitude))==201 and len((((cm[i])[1]).altitude))==201 and len((((cm[i])[0]).common_o3_vmr))==201 and len((((cm[i])[1]).o3_vmr))==201 and len((((cm[i])[1]).is_retrieved))==201 and len((((cm[i])[0]).common_o3_vmr_error))==201 and len((((cm[i])[1]).o3_vmr_error))==201 :
             print(i)
             ...
             while curr_altitude<=max_alt:
                 ...
                 if ((cm[i])[1].is_retrieved)[idx1]==1 and numpy.isnan(((cm[i])[0].common_o3_vmr)[idx0])==False and numpy.isnan(((cm[i])[0].common_o3_vmr_error)[idx0])==False :#and ((cm[i])[1].common_is_retrieved)[idx0]==1 : # Check that we have only valid numbers.
